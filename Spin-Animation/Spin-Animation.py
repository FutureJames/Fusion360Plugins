        ...
        try:
            ...
            for input in inputs:
                if input.id == 'filename':
                    frameRecorder.filename = input.value.replace(" ", "_")
            ...
            #generate helper script for running FFMPEG
            frameRecorder.createFfmpegScript(frameRecorder._toolName, frameRecorder.outputPath, frameRecorder.filename)
...
class CommandCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            dropdownItems.add('Y', False)  
            dropdownItems.add('Z', True) # Default to Z Up
...
class FrameRecorder:
    def __init__(self):
        ...
        # Set initial values.
        self._filename = dataFile.name.replace(" ", "_")
        self._outputPath = os.path.expanduser("~/Desktop/")
        self._toolName = "Spin-Animation_"
        self._width = 2000
        self._height = 2000
        self._framesPerRotation = 25
        self._numRotations = 1
        self._upAxis = RotationAxis.Z
        ...
        #replace deprecated property access
        (returnValue, width, height) = camera.getExtents()
        self._zoomStart = width 
    ...
    def collectFrames(self):
        ...
        for i in range(0, numFrames):
            ...
            # Save image.
            success = app.activeViewport.saveAsImageFile(outputPath + toolname + filename + '/' + filename + '_' + str(i) + '.png', width, height)
            if not success:
                ui.messageBox('Failed saving viewport image.')


    #helper script for rendering the frames in to video 
    def createFfmpegScript(self, toolname, outputPath, filename):
        try:

            #Generate a batch script the uses FFMPEG to create the  animation
            batFile = open(outputPath + toolname + filename + '/renderAnimation.bat', 'w')
            # Assemble png files into mp4 video
            batFile.write('ffmpeg -r 30 -y -i ' + filename + '_%%d.png -c:v libx264 -preset slow -crf 22 -pix_fmt yuv420p -an ' + filename + '.mp4 ' + "\n")
            # Create animated gif from previous step mp4 video
            batFile.write('ffmpeg -y -i ' + filename + '.mp4 -vf "fps=10,scale=320:-1:flags=lanczos,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 ' + filename + '.gif' + "\n")
            # Selete the static png files and leave the animations
            batFile.write("del *.png")
            batFile.close()
        except:
            ui.messageBox('Failed to save batch file.')
