...
class VersionTimelapse:
    def __init__(self):
        ...
        self._toolName = "Version-Timelapse_"
        ...
        self._rotate = False # Add model rotation to exported frames.
    ...
    def collectFrames(self):
        ...
        for i in range(start, end):
            ...
            # Save image.
            success = viewport.saveAsImageFile(outputPath + toolname + filename + '/' + filename + '_' + str(i + 1) + '.png', width, height)
            if not success:
        ...
        for j in range(finalFrames):
            ...
            # Save image.
            success = app.activeViewport.saveAsImageFile(outputPath + toolname + filename + '/' + filename + '_' + str(i + j + 2) + '.png', width, height)
            if not success:
                ui.messageBox('Failed saving image for ' + version.name + ' version ' + version.versionId.split('version=')[1])
...
To change animation parameters, you will need to modify the code directly - see github.com/amandaghassaei/Fusion360-Design-Version-Timelapse for more details.
        ...
        timelapse.createFfmpegScript(timelapse._toolName, timelapse._outputPath, timelapse._filename)
